...
namespace Microsoft.Extensions.Hosting
{
    internal class HostFactoryResolver
    {
        ...
        public static readonly string CreateBuildWebHost = nameof(CreateBuildWebHost);
        ...
        public static Func<string[], TWebHostBuilder> ResolveCreateBuildWebHostFactory<TWebHostBuilder>(Assembly assembly)
        {
            return ResolveFactory<TWebHostBuilder>(assembly, CreateBuildWebHost);
        }
        ...
        public static Func<string[], IServiceProvider> ResolveServiceProviderFactory(Assembly assembly)
        {
            ...
            var hostCreateBuilderFactory = ResolveCreateBuildWebHostFactory<object>(assembly);
            if (hostCreateBuilderFactory != null)
            {
                return args =>
                {
                    var hostBuilder = hostCreateBuilderFactory(args);
                    var host = Build(hostBuilder);
                    return GetServiceProvider(host);
                };
            }
